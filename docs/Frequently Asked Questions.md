...
It removes every file and symlink the project deployed (libraries, shims, SSoT, service, desktop entries, pacman hook), restores the boot chain (getty on TTY1, your display manager, the default target), resets the Gamescope capabilities, and optionally wipes your user configs. Three things are deliberately **left in place**: the installed packages (`steam`, `gamescope`, Mesa drivers, `kate`, `konsole`…), the `[multilib]` repository, and the groups your user was added to — they are standard system components, not project files, and removing them could break software unrelated to the project.
